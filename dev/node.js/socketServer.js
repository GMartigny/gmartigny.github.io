...
function Player(con){
    ...
    this.pseudo = false;
    this.details = {
        anim: 1, dir: 0, pos: {}, map: "", mess: [], globalPos: {}
    };
    ...
    this.getOthers = function (){
        ...
        for(var i = 0; i < players.length; ++i){
            ...
            if(one.pseudo && one.pseudo !== this.pseudo && this.inRange(one)){
                arr.push({
                    pseudo: one.pseudo, d: one.details
                });
            }
        }
    ...
    this.inRange = function (other){
        if(other.details.map == this.details.map &&
                ...
                other.details.pos.y - 10 < this.details.pos.y && other.details.pos.y + 10 > this.details.pos.y)
            return true;
        else
            return false;
    };
